...
def main():
    ...
    poses = hba.finish()
    st = np.array(hba.stats()).reshape(-1, 7)   # round, start_scan, kfs, iters, ba_ms, pgo_ms, done_scan
    ...
    print(f'rounds {len(st)} (used {(st[:, 6] >= 0).sum()}), ba s last {st[-1, 4] / 1e3 if len(st) else 0:.1f}, '
